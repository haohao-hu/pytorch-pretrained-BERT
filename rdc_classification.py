        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if (step+1)%100==0:
                    ...
                    #info={'loss':loss.item()}
                    datachartpoint={"chart": "live training loss", "y": tr_loss/nb_tr_steps, "x": step+1}
                    print(json.dumps(datachartpoint))
                    '''
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        # Run prediction for full data
            ...
            for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
                ...
                label_ids=to_np(label_ids)
                targs.append(label_ids)
                tmp_eval_accuracy = accuracy(logits, label_ids)
                #testing:
                '''
                for tune_f1 in False, True:
                    pred = predict(np.concatenate(probs), tune_f1=tune_f1)
                    p,r,f1=score(pred, np.concatenate(targs))
                    print('Tune f1:{}, weighted-p,r,f1:{:.4f},{:.4f},{:.4f}'.format(str(tune_f1),p,r,f1))
                #end testing
                '''
            ...
            #probs = logprob_scale(probs)
            eval_loss = eval_loss / nb_eval_steps
            eval_accuracy = eval_accuracy / nb_eval_examples
            loss = tr_loss/nb_tr_steps if args.do_train else None

            for tune_f1 in False, True:
                pred = predict(probs, tune_f1=tune_f1)
                p,r,f1=score(pred, targs)
                print('Testset:{}, Tune f1:{}, weighted-p,r,f1:{:.4f},{:.4f},{:.4f}'.format(str(is_test),str(tune_f1),p,r,f1))
                result = {'Test set or not':is_test,
                          'Tune F1 or not':tune_f1,
                    'eval_loss': eval_loss,
                ...
                output_eval_file = os.path.join(args.output_dir, "testset-{}-tunef1-{}-eval_results.txt".format(str(is_test),str(tune_f1)))
                with open(output_eval_file, "w") as writer:
                    logger.info("***** Eval results *****tune-f1:{} on Testset:{}".format(str(tune_f1),str(is_test)))
